...
BEGIN {
    ...
    $ENV{TERM}="xterm";
    $ENV{PATH} = "$ENV{METAAMPHOME}/bin:$ENV{METAAMPHOME}/programs:$ENV{METAAMPHOME}/programs/mothur:/gpfs/ebg_data/programs/cutadapt/bin:/gpfs/ebg_data/programs/R/bin:$ENV{PATH}";
...
########################param for both single and paired input
...
my $truncq = $query->param("truncq");

####################params for paired
my $maxee1 = $query->param("maxee1");
my $maxee2 = $query->param("maxee2");
my $truncq1 = $query->param("truncq1");
my $truncq2 = $query->param("truncq2");
my $trunclen1 = $query->param("trunclen1");
my $trunclen2 = $query->param("trunclen2");
#paired merge
...
my $cmd = $seqtype eq "paired" ? "metaamp.dada2.paired.pl -map $upload_mapping_file -an $analysis -seqtype $seqtype -oligos oligos.txt -pdiffs $pdiffs -email $email -maxN $maxn -maxeeF $maxee1 -maxeeR $maxee2  -trunclenF $trunclen1 -trunclenR $trunclen2 -truncqF $truncq1 -truncqR $truncq2 -minoverlen $minoverlen -maxdiffs $maxdiffs  >& debug.txt;" : "metaamp.dada2.single.pl -map $upload_mapping_file -an $analysis -seqtype $seqtype -trunclen $trunclen -maxee $maxee -truncQ $truncq -oligos oligos.txt -pdiffs $pdiffs -email $email  >& debug.txt;";
...
sub extract_archive {
	...
	for my $file (@{$ae->files}) {
		...
		print_error_web("MetaAmp asv analysis only acceipt fastq format sequences. Your uploaded files are not having fastq or fastq.gz extenson",
				"Please ensure your file has extension as \".fastq, .fastq.gz\"",
