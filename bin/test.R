...
#path<-"opt$seqdir/PrimTrim/"
path<- opt$seqdir
seqtabname = opt$aname
...
filtFs <- file.path(path, paste0(sample.names, "_R1.filt.fastq.gz"))
filtRs <- file.path(path, paste0(sample.names, "_R2.filt.fastq.gz"))
filtFs
filtRs
...
out
write.table(gsub("^(./+)", "", c(unname(filtFs),unname(filtRs))), file="dada2_filter_trim_file.list.txt", row.names = FALSE, na='', col.names = FALSE,  quote=F)
...
#write.csv(seqtab.nochim,file="mock.asv.csv")
...
write.table(test, file=paste0(seqtabname, ".shared"), sep="\t", row.names = FALSE, quote=FALSE)

# Write a fasta file containing the seq variants
library(seqinr)
write.fasta(uniqueSeqs, fasta_header, paste0(seqtabname,".asv.fasta"))
