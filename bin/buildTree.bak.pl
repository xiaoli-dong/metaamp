...
($taxontxt) ||
    die "usage: $0 OPTIONS where options are: -i  <input file name: taxonmy summary file> -dir <output directory>\n";

my $bin = "$FindBin::RealBin";

my $jsonfile = "$dir/$taxontxt.json";
my $cmd = "$bin/taxons2jsontree.pl -i $dir/$taxontxt -d 3 > $jsonfile";
system($cmd);

my $taxontree = "$dir/$taxontxt.html";
print_taxonTree($taxontree);

my $taxonjs = "$dir/$taxontxt.js";
print_taxonjs($taxonjs, "$taxontxt", $dir);
...
sub print_taxonjs{
    ...
    my ($taxonjs, $taxontxt, $dir) = @_;
    open(INPUT, "<$dir/$taxontxt") or die $!;
