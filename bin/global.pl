...
sub getTaxonInfo{
    ...
    $cmd .= "$bin/getTaxonSummary.pl -i1 $analysisname.$prefix.taxonomy -i2 $analysisname.shared -l $label > $analysisname.taxonomy.summary.txt;";
...
sub send_job_finished_email {
   ...
   $analysis =~ s/Job name://;
   $email =~ s/Email://;
   $human_time =~ s/Submit human time://;
   $machine_time =~ s/Submit machine time://;

   print STDERR "*******************$analysis, $email, $human_time, $machine_time\n";
