...
domain_x = d3.set(data.map(function(d) {
    return d.colid
})).values(),
domain_y = d3.set(data.map(function(d) {
    return d.rowid
})).values();
...
var colLabels = svg.append("g")
    ...
    .append("text")
    .text(function (d) { return d; })
    .attr("y", function(d, i){return x(d) + cellsize/2})
    .attr("x", "0")
...
var cell = svg.selectAll(".cell")
    ...
    .enter()
    .append("g")
    //.attr("class", function(d){return "cor cell cell-border cr_"+ d.rowid +" cc_"+ d.colid })
...
cell.append("rect")
    ...
    .attr("class", function(d){return "rect rr_" + d.rowid + " rc_"+ d.colid;})

cell.filter(function(d){
    //data object
    var ypos = domain_y.indexOf(d.rowid);
    var xpos = domain_x.indexOf(d.colid);
    //for (var i = (ypos + 1); i < num; i++){
    for (var i = (ypos + 1); i < rcount; i++){
...
cell.filter(function(d){
    ...
    .attr("cx", function(d) {return x(d.colid) + cellsize/2})
    .attr("cy", function(d) { return y(d.rowid) + cellsize/2})
    .attr("r", function(d){
        //return cellsize * (Math.abs(d.value) + 0.1);
	return cellsize/1.5 * Math.sqrt(Math.abs(d.value));
    })
    ...
    .style("fill", function(d){
	    ...
	    //console.log("color=" + chromaScale(d.value));
            return color(d.value);
	    //return chromaScale(d.value);
        }
    })
...
function sortbylabel(rORc,element,sortOrder){
	...
	t.selectAll(".cell")
		...
		return xsorted(d.colid) + cellsize/2;
		
	    });
	t.selectAll(".rect")
	    .attr("x", function(d, i) { 
		return xsorted(d.colid)  
		
	    });
	t.selectAll(".colLabel")
	    .attr("y", function (d, i) { return sorted.indexOf(i) * cellsize + cellsize/2});
	...
	t.selectAll(".cell")
	    .attr("cy", function(d) { 
		return ysorted(d.rowid) + cellsize/2;
	    });
	
	t.selectAll(".rect")
	    .attr("y", function(d) { 
		return ysorted(d.rowid);
	    });
...
function order(value, rSortOrder, cSortOrder){
	...
	t.selectAll(".cell")
	    .attr("cy", function(d) { return ysorted(d.rowid) + cellsize/2});
	t.selectAll(".rect")
	    .attr("y", function(d) { 
		return ysorted(d.rowid);
	    });
	...
	t.selectAll(".cell")
	    .attr("cx", function(d, i) { return xsorted(d.colid) + cellsize/2 });
	t.selectAll(".rect")
	    .attr("x", function(d, i) { return xsorted(d.colid)});
	t.selectAll(".colLabel")
	    .attr("y", function (d, i) { return clabels_sorted_index.indexOf(i) * cellsize + cellsize/2; });
